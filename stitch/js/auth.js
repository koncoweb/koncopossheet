...
// ============================================================
// CSRF TOKEN — fetch token for login/register
// ============================================================
async function fetchCsrfToken() {
  try {
    const r = await gasRequest({ query: { action: 'getCsrfToken' } });
    return r.csrf_token || null;
  } catch (e) { return null; }
}
...
async function doLogin() {
  ...
  try {
    ...
    const result = await gasRequest({
      body: { action: 'login', email, password, csrf_token: csrf || '' }
    });
...
async function doRegister() {
  ...
  try {
    const csrf = await fetchCsrfToken();
    const result = await gasRequest({
      body: { action: 'register', namaLengkap: nama, namaUsaha, jenisUsaha, telp, email, password, csrf_token: csrf || '' }
    });
