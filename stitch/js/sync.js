...
async function gasRequest(params) {
  ...
  if (isPost) {
    ...
    if (result.code === 429) {
      const waitSec = result.retryAfter || 5;
      if (params._retryCount < 2) {
        params._retryCount = (params._retryCount || 0) + 1;
        await new Promise(r => setTimeout(r, waitSec * 1000));
        return gasRequest(params);
      }
      throw new Error(result.error || 'Terlalu banyak permintaan');
    }
    ...
    // Handle rate limit (429) for GET too
    if (result.code === 429) {
      const waitSec = result.retryAfter || 5;
      if (params._retryCount < 2) {
        params._retryCount = (params._retryCount || 0) + 1;
